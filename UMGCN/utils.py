...
def load_data(config):
    ...
    idx = np.loadtxt(config.idx_path, dtype = int)
    ...
    return features, label, idx_train, idx_test,idx
def load_data1(config):
...
def load_graph(dataset, config):
    ...
    return nsadj, nfadj
def load_graph1(dataset, config):
    featuregraph_path = config.featuregraph_path + str(config.k) + '.txt'
...
def load_mixorderadjs(adj):

    adj2 = sparse.coo_matrix(adj * adj)
    adj3 = sparse.coo_matrix(adj2 * adj)
    # adj4 = sparse.coo_matrix(adj3 * adj)

    adj11 =  sparse_mx_to_torch_sparse_tensor(adj)
    adj22 = sparse_mx_to_torch_sparse_tensor(adj2)
    adj33 = sparse_mx_to_torch_sparse_tensor(adj3)
    # adj44 = sparse_mx_to_torch_sparse_tensor(adj4)


    return adj11, adj22, adj33
